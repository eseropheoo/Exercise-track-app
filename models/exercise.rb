class Exercise
    attr_accessor :title, :id, :username, :target_muscles, :body, :days_trained, :diet, :reps
    ...
    def self.all
        ...
        sql = "SELECT title,id,username,target_muscles, body,days_trained,diet,reps FROM fit ORDER BY id"
    ...
    def self.find id
        conn = self.open_connection
        sql = "SELECT title,id,username,target_muscles,body,days_trained,diet,reps FROM fit WHERE id = #{id}"
        results = conn.exec(sql)
